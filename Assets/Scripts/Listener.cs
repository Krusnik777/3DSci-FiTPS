    ...
    public class Listener : MonoBehaviour
    {
        ...
        private bool HearTargetInCloseRange(ListenTarget target)
        {
            if (target.IsHearableFromPoint(transform.position, m_listenZones.CloseRange))
            {
                ...
                var shooter = target.GetComponentInChildren<PlayerShooter>();

                if (shooter == null) Debug.LogError("ListenTarget: PlayerShooter is not found");

                if (targetMovement != null)
                    if (!targetMovement.IsNotMoving) return true;

                if (shooter != null)
                    if (shooter.IsFired) return true;
            }
